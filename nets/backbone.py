...
class CIF(nn.Module):
    def __init__(self, num_classes=21, last_inp_channels=720):
        super(CIF, self).__init__()
        channels_up = 512
        self.attention = nn.Sequential(nn.Conv2d(channels_up, channels_up, 3, padding=1),
                                               nn.Sigmoid())
        self.attention_1 = nn.Sequential(nn.Conv2d(channels_up, channels_up, 3, padding=1),
                                               nn.Sigmoid())
        self.last_layer_e = nn.Sequential(
                nn.Conv2d(in_channels=channels_up, out_channels=channels_up, kernel_size=1, stride=1,
                          padding=0),
                nn.BatchNorm2d(channels_up, momentum=BN_MOMENTUM),
                nn.ReLU(inplace=True),
                nn.Conv2d(in_channels=channels_up, out_channels=num_classes[0], kernel_size=1, stride=1, padding=0)
        )
        self.last_layer_d = nn.Sequential(
                ...
                nn.BatchNorm2d(channels_up, momentum=BN_MOMENTUM),
                nn.ReLU(inplace=True),
                nn.Conv2d(in_channels=channels_up, out_channels=num_classes[1], kernel_size=1, stride=1, padding=0)
        )

    def forward(self, x, x_1):
        x_1_features = x_1
        x_1_mask = self.attention_1(x_1)
    
        x_features = x
        x_mask = self.attention(x)
    
        x_fuse = x + torch.mul(x_1_features, x_mask)
        x_1_fuse = x_1 + torch.mul(x_features, x_1_mask)
    
        x_fuse = self.last_layer_e(x_fuse)
        x_1_fuse = self.last_layer_d(x_1_fuse)
    
        return x_fuse, x_1_fuse